...
*最一般地，刚体的任意运动是平动和转动的叠加，这就是Chasles定理*。刚体的平动可以选一个代表点(刚体上任意点)描述，其他点相对这个代表点的运动其实是转动。这个代表点的不同选择从运动学的角度来看是完全等价的(虽然代表点的不同选择可能导致颇为不相同的平动)，然而从动力学的角度来看，选质点组的*质心*为代表点将使动力学方程得到简化，因而往往优先选用。就转动部分而言，绕定点的转动是最一般的情况，定轴转动显然是其特殊情况。

在定轴转动情形，经常用类比的方法处理，即它与一维平动运动规律相对应，但这种类比不能任意推广到定点转动。典型的例子是，自由平动物体的速度不变，但自由定点转动刚体的角速度有可能随时间变化；平动物体的动量与速度同向，但*一般刚体运动的角速度与角动量经常是不同向的*。原因是转动与平动有本质的差异，平动惯性相关的是*质量标量*，而与转动惯性相关的是转动*惯量张量*。
...
$
  vb(a)_i = vb(a)_0 + dot(vb(omega)) times vb(r)_i + vb(omega) times dot(vb(r)_i) = vb(a)_0 + dot(vb(omega)) times vb(r)_i + vb(omega) times (vb(omega) times vb(r)_i)
$
...
#proof[
  ...
  $
    U vb(X) = lambda vb(X)
  $
  其中$lambda$为本征值，$vb(X)$为本征矢。本征值可由行列式方程求解
  $
    det(U - lambda I) = 0
  $
  其中$I$是单位矩阵。展开得到
  $
    - lambda^3 + b lambda^2 + c lambda + 1 = 0
  $
  其中
  $
    b & = tr(U) \
    ...
    1 & = det(U)
  $
  实系数三次方程必然至少有一个实根(三次函数值域是从负无穷到正无穷，必然要至少通过实轴一次)。假设这个实根为$a$，则根据三次方程的Viète定理容易证明另两个根互为复共轭$xi,xi^*$(包含两个复数根都是实数的情形)，且$a xi xi^* = 1$。

  假设实根$a$对应的本征矢为$vb(B)$，则本征方程为
  $
    U vb(B) = a vb(B)
  $
  左侧代表对$vb(B)$矢量的转动，这个矢量经过转动变成$a$倍。而矢量经过转动模不变，因此这个$a$只可能是$plus.minus 1$。而两个复共轭根乘在一起一定大于零，因此必然只能$a = 1$，且$xi xi^* = 1$。于是有
  $
    a= 1, xi = e^(i phi)
  $
  #newpara()
  先讨论$phi != 0,pi$的情形。若本征值$e^(i phi)$对应的本征矢为$vb(C)$，得到
  $
    U vb(B) = vb(B), U vb(C) = e^(i phi) vb(C), U vb(C)^* = e^(-i phi) vb(C)^*
  $
  考虑到
  $
    vb(B)^TT U^TT U vb(C) = vb(B)^TT vb(C) = e^(i phi) vb(B)^TT vb(C)
  $
  从而
  $
    vb(B)^TT vb(C) = 0
  $
  类似地有$vb(B)^TT vb(C)^* = 0$，因此$vb(B)$与$vb(C), vb(C)^*$正交。又由于$vb(C), vb(C)^*$是复共轭，因此可以构造出两个实矢量
  $
    vb(C)_1 = vb(C) + vb(C)^*, vb(C)_2 = i(vb(C) - vb(C)^*)
  $
  显然有$vb(B)^TT vb(C)_1 = 0, vb(B)^TT vb(C)_2 = 0$，因此$vb(B), vb(C)_1, vb(C)_2$构成一组正交基底。并且
  $
    U vb(C)_1 = cos phi vb(C)_1 - sin phi vb(C)_2\
    U vb(C)_2 = sin phi vb(C)_1 + cos phi vb(C)_2
  $
  以上数学结果可以直观理解：$vb(B)$是转动不变的方向，即转动轴方向，$vb(C)_1, vb(C)_2$是垂直于转动轴的两个方向，在这两个方向上，矢量经过转动会绕转动轴旋转一个角度$phi$(与直角坐标系绕$z$轴转动$phi$角的结果类似)。

  若$phi = pi,xi = xi^* =—1$，两个本征值简并，可以直接要求两个本征矢正交。若$phi =0$，三个本征值简并，直接要求三个本征矢相互正交。当然此时对应无转动或者转一周情形。两种特殊情形与$phi$取一般值情形结论无异。
]

=== Euler角

据Euler定理，可以用2个方位角确定某一轴的方向，然后另外1个角表示绕该轴的转动角度，这样3个角度完全确定刚体的方位，亦即3个角度完全描述刚体的定点转动。

刚体的定点转动有3个自由度，刚好可以由3个角度代表的广义坐标所描述。比较典型的方法是利用3个Euler角。这就给出了描写刚体转动的广义坐标。
...
刚体的任何转动都可以通过3个Euler角的转动实现。对于刚体绕瞬时轴无限小角度转动，可以通过3个无限小Euler角的转动达到，这样*刚体的角速度实际上可以用3个Euler角的变化率描述*。
...
在实验室系中
$
  vb(omega') = mat(omega'_1; omega'_2; omega'_3) = U^TT_1 mat(dot(theta); 0; dot(phi)) + U^TT_1 U^TT_2 mat(0; 0; dot(psi)) = mat(
    dot(theta) cos phi + dot(psi) sin phi sin theta;
    dot(theta) sin phi - dot(psi) cos phi sin theta;
    dot(phi) + dot(psi) cos theta
  )
$
在本体参考系中
$
  vb(omega) = mat(omega_1; omega_2; omega_3) = U_3 U_2 mat(dot(theta); 0; dot(phi)) + U_3 mat(0; 0; dot(psi)) = mat(
    dot(theta) cos psi + dot(phi) sin psi sin theta;
    -dot(theta) sin psi + dot(phi) cos psi sin theta;
...
设实验室参考系$K'$为惯性参考系，实验室系刚体*质心*位矢为$vb(R)$，本体坐标系$K$的原点选在质心，刚体角速度为$vb(omega)$，刚体上任意点$i$的坐标为
$
...
考虑一质量$M$的刚体*绕某个定点转动*，选该定点作为*本体坐标系$K$的原点*，此时若*质心在$vb(a)$处*，相对于质心的位矢若用$vb(r)'_i$表示，则$vb(r)'_i + vb(a) = vb(r)_i$则相对于原点的角动量可以写成
$
  vb(J) & = sum_i Delta m_i vb(r)_i times (vb(omega) times vb(r)_i) \
...
$
  cal(I)' = cal(U) cal(I) cal(U)^TT => (cal(I) - cal(I)' ) cal(U)^TT = 0 => sum_beta (I_(alpha beta) - I_k delta_(alpha beta)) U_(k beta) = 0
$
...
*物理上寻找刚体的主轴和数学上把转动惯量张量的矩阵对角化是一回事*。转动惯量张量的主轴体现了刚体质量分布的某种*对称性*。如果刚体的密度是常数，很容易从刚体的几何对称性直接找到主轴。如果刚体的形状没有对称性，且刚体的质量密度分布不规则，则刚体的主轴不容易直观地直接找到。但是实对称矩阵的对角化总是可能的，因此主轴一定是存在的，只是要通过计算找到。
...
为了使方程形式简单，选用*惯量主轴本体坐标系*，坐标系原点选在质心或固定点，以保证转动惯量张量为常量，且角动量定理有简单的形式。假设外力矩为$vb(M)$，选本体坐标轴为惯量主轴(即惯量积为零)，角动量定理具体表示为
$
...
自由转动或惯性转动是指*外力矩为零*的情形。在重力场中刚体的定点自由转动，定点一定与质心重合，否则重力矩不能为零。如果是一般运动，可以分别考虑刚体质心的平动和绕质心的转动，重力矩自动为零。
...
$
即动能守恒。此时在$vb(omega)$空间表示椭球面，这个特定曲面叫*惯性椭球面*(inertia ellipsoid)，与惯量椭球面类似，只是长短轴的长度变成了$sqrt((2 T)/I_k)$。*$vb(omega)$的端点在椭球面上，椭球面上任一点到球心的距离，表示角速度的大小。*
...
$
  I_1 = I_2 != I_3
$
*惯量椭球面是对第三轴对称的一个旋转椭球面*。这个椭球面在Poinsot面上滚动时，以椭球中心与Poinsot面距离保持不变的形式作纯滚动时，容易看出*本体瞬心迹和空间瞬心迹都是圆*，亦即运动过程圆锥母线长和高都不变，代表角速度大小和它的第三轴分量也都不变。
...
作为例子，考查惯量椭球为扁椭球情形($n>0,I_3>I_1=I_2$)。由于角动量守恒，可以*假设角动量方向沿实验室系$z$轴方向*，这样可以简化问题。在*本体坐标系*，角速度的3个分量为
$
  vb(omega) = mat(
    omega_perp cos n t;
    omega_perp sin n t;
...
#example(subname: [])[
  有一匀质的薄圆盘，质量$m$，半径$r$，盘本身绕自身对称轴$omega_0$自转，其对称轴绕竖直轴以恒角速度$Omega$转动，圆盘对称轴与竖直方向角度$alpha$不变，竖直轴穿过圆盘中心O点。选择O点作为原点，本体坐标系$z$轴与圆盘对称轴重合，$x-y$轴选在圆盘面内，选竖直方向做实验室坐标系$z'$轴，且在$y-z$平面内。求：
  + 本体坐标系下的角速度
  + 本体坐标系下的动能、角动量
  + 本体系O点的力矩
  + 实验室系O点的力矩、角动量
...
- $f(u)$在区间$[-1,+1]$有一个实根$u_1$有解的可能情形
  ...
  - $u_1 = 1$是$f(u)$的根又是拐点，因此$f(1)=0, f'(1)=0, f''(1)=0$即这个根是三重根。这要求$a=b, alpha = beta$另外满足$a^2/beta = 2$。此时$theta = 0, evaluated(dot(theta))_(theta=0) = 0, evaluated(dot.double(theta))_(theta=0) = 0$，因此*陀螺将保持直立转动*的不稳定运动，会逐渐恢复到$theta = 0$的状态，之后再次偏离，如此反复
  - $u_1 = cos theta_1$是切点，因此是重根。此时
    $
      f(u) = beta (u - u_1)^2 (u - u_2)
    $
    容易得到$theta = theta_1, evaluated(dot(theta))_(theta=theta_1) = 0, evaluated(dot.double(theta))_(theta=theta_1) = 0$，因此*因此陀螺作无章动的进动*，进动角速度为
    $
      evaluated(dot(phi))_(theta=theta_1) = (b - a cos theta_1) / (sin^2 theta_1)
    $
